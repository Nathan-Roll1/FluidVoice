...
final class ASRService: ObservableObject {
    ...
    private func registerDeviceListChangeListener() {
        ...
        let token: AudioObjectPropertyListenerBlock = { [weak self, weak controller] _, _ in
            #if DEBUG
                AudioTopologyDiagnostics.record(.callback, owner: .asrDeviceList, objectID: AudioObjectID(kAudioObjectSystemObject), selector: kAudioHardwarePropertyDevices, scope: kAudioObjectPropertyScopeGlobal, element: kAudioObjectPropertyElementMain, queueRole: .dedicatedDelivery)
            #endif
